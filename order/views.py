...
class OrderListCreateAPIView(APIView):
    def get(self,request):
        orders=Order.objects.al()
        serializers=OrderSerializer(orders,many=True)

        for order_data in serializers:
    ...
    def post(self,request):
        ...
        if serializer.is_valid():
            serializer.save()
            msg="Orders's Successfully" if isinstance(data,list) else "Order Successfully"

            return Response({"result":serializer.data, "message":msg},status=status.HTTP_201_CREATED)
    ...
    def delete(self,request):
        ...
        try:
            ...
            return  Response({"messgage":msg, "error":str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class OrderDetailAPIView(APIView):
    ...
    def put(self,pk):
        try:
            # assuming 'pk' is either the user id or food id
            i_id=int(pk) 

            # check if pk is valid user id

            if Order.objects.filter(account_id=i_id).exists():
                orders=Order.objects.filter(account_id=i_id)

                # upload all orders for user
                for order in orders:
                    order.complete=True
                    order.save()
                return Response({"message":"All orders updated successfylly!"})
            
            # chek if 'pk' is a valid food id
            elif Order.object.filter(id=i_id).exists():
                # updata the specific order by setting 'complete to true'

                order=Order.objects.get(id=i_id)
                order.complete=True
                order.save()
                return Response({'message':"Order updated successfully!"})
            return Response({'error':'Invalid user id or food id'})
        except ValueError:
